...
start_yr = 3535; inputRestartFileStem = 'restart_0_1_output/restart_3536_Fe_x1.mat';
...
if( sim.forwardIntegrationOnly )
    ...
    atol   = norm(x0)*1e-2; % stop if norm(drift) < 1ppm of norm(x0)
    ...
    bgc_sol = bgc;
    nsoli_sol = replaceSelectedTracers(sim, c0, sol, sim.selection);
    ...
    % Copy restart file phi() dropped and label it the solution file...
...
%         it_histx(itc,1) = norm(r);
    ...
    % If we relaxed sol, x is a relaxed x1.
    %   if we did NOT rleax then x is sol = x0
    bgc_relax = bgc;
    ...
    for fwd_itc = 1:num_forward_years
        ...
        if mod(years_gone_by, sim.yearsBetweenRestartFiles) == 0    % This runs after last time step of every 10 y
            ...
            [sim, bgc_fwd] = saveRestartFiles(sim, bgc_fwd, tracer_0, 0);
        end
    end % fwd loop
