...
!        because its tripping over "-fmax-stack-var-size" which then causes recursion...
  integer, parameter          :: MAX_LOG_ENTRY_CNT = 10 *1024
  ! char_len could be 384 or 512... 
  ! 	so MAX_STR_LEN could be as much as 5M, which is nuts...
...
! =============================================================================
  ...
  function init_marbl(delta_z, zw, zt, nlev, nt)
    ...
    call marbl_instance%init(gcm_num_levels = INT(size(delta_z),4),&
                             gcm_num_PAR_subcols = INT(1,4), &
                             gcm_num_elements_surface_flux = INT(1,4), &
                             gcm_delta_z = delta_z, &
                             gcm_zw = zw, & 
                             gcm_zt = zt, &
                             lgcm_has_global_ops = LOGICAL(.true.,4))
...
!    character(len=char_len)                     :: input_line, varname
!    , sname
...
!      call marbl_instance%inquire_settings_metadata(n, sname=varname)
      ...
      ! call marbl_instance%get_setting(varname, input_line, lsettings_file_format=.true.)
!      call marbl_instance%get_setting(varname, input_line, lsettings_file_format=.true.)
      if (marbl_instance%StatusLog%labort_marbl) exit
