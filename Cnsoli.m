function [sol, it_hist, ierr, x_hist] = Cnsoli(x,f,tol, parms)
...
if nargin == 4
    ...
    if length(parms) == 5
       % modified 5/11/18 to always orthogonalize in dgmres (3 instead of 1)
       gmparms = [abs(etamax), lmaxit, parms(5), 3]; 
    end
end
%
...
[dc,f0,x] = f(x,Npt);
if dc == false
  disp('data constrained is false for initial values');
  sol = 0; it_hist = 0; ierr = 10; x_hist = 0;
  return
end
fnrm = norm(f0);
...
%
    ...
    Npt = 1;
    disp(['itc = ',int2str(itc), ...
              ', step max = ',num2str(max(step),3), ...
	      ', step min = ',num2str(min(step),3), ...
	      ', lambda = ',num2str(lambda,3)]);
    xt = x + lambda*step;
    [dc,tft,xt] = f(xt,Npt);       
    if dc == true, ft = tft; end; 
...
%
% Keep xt within data constraints
%
       disp(['itc = ',int2str(itc), ...
	          ', step max = ',num2str(max(step),3), ...
       		  ', step min = ',num2str(min(step),3), ...
...
%
end % while(fnrm > stop_tol & itc < maxit)
sol = x;
it_hist = it_histx(1:itc+1,:);
if debug == 1
    disp(outstat)
...
%
dc = false; dc_cnt = 0; dc_cnt_max = 50;
while (dc == false) & (dc_cnt <= dc_cnt_max)
    dc_cnt = dc_cnt + 1; feps = 1/dc_cnt;
    del = x+feps*epsnew*w;
    disp(['in dirder', ...
              ', epsnew = ',num2str(epsnew,3), ...
              ', feps = ',num2str(feps,3)]); 
[dc,tf1,del] = f(del,Npt);
if dc == true, f1 = tf1; end;
end % while dc == false
if dc == false
    disp('in dirder, dc count exceeded')
    keyboard
end
z = (f1 - f0)/(feps*epsnew);
...
total_iters = 0;
if(rho < errtol) 
    disp(' early termination in dgmres')
return
end
%
%
